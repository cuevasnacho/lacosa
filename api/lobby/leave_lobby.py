...
async def leave_lobby(lobby_id : int, player_id : int):
    ...
    #y se actualiza el estado de todos los jugadores.
    ...
    if not Player[player_id].player_isHost:
        with db_session:
            ...
            lobby_update(lobby_id, lobby.lobby_match.match_id)
            commit()    
    else:
        with db_session:
            if Player[player_id].player_isHost and lobby.lobby_pcount > 0:
