...
def test_leave_lobby_host_no_players():
    ...
    player_mock.player_ingame = True  # Simula que el jugador no está en juego.

    # Configurar un mock de Player para un jugador que es el host y no hay otros jugadores

    #query = select(l for l in Lobby)
    #print(f'queri del lobby:')
    #print(query[:])
    ...
    with patch("api.lobby.leave_lobby.get_lobby", return_value = lobby_mock),\
        ...
        assert response.content == "El objeto no existe"
#------------------------------------------------------------------------------------------------------------------#
